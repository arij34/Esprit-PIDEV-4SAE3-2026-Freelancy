...
<!-- template: login.ftl (freelancy override) -->
        ...
        <#if realm.password && realm.registrationAllowed && !registrationDisabled??>
            <div id="kc-registration-container">
                <div id="kc-registration">
                    <#-- Redirect register link to Angular signup page -->
                    <#assign angularBase = properties.angularBaseUrl!"http://localhost:4200" />
                    <span>${msg("noAccount")} <a href="${angularBase}/signup">${msg("doRegister")}</a></span>
                </div>
            </div>
        </#if>
    </#if>
