...
export class FreelancerSkillService {
 ...
 // forceCreate = false par défaut
createWithSkillInput(skillInput: string, payload: Partial<FreelancerSkill>, forceCreate = false): Observable<any> {
  return this.http.post<any>(
    `${this.url}/user/me?skillInput=${encodeURIComponent(skillInput)}&forceCreate=${forceCreate}`,
    payload
  );
}
 ...
 update(freelancerSkill: FreelancerSkill): Observable<any> {
  console.log('🔷 [DEBUG update] URL:', `${this.url}/user/me/${freelancerSkill.id}`);
  return this.http.put<any>(
    `${this.url}/user/me/${freelancerSkill.id}`,
    freelancerSkill
  );
}
