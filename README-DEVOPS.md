# blog-analytics-service DevOps Guide
...
docker build -t pidevfreelancy/blog-analytics-service:latest .
docker run --name blog-analytics-service -p 8053:8053 pidevfreelancy/blog-analytics-service:latest
```

Cleanup:

```bash
docker stop blog-analytics-service
docker rm blog-analytics-service
```
...
- this pipeline automatically uses `/home/vagrant/DEVOPS/blog-analytics-service` if that shared folder exists
...
minikube service blog-analytics-service -n freelancy --url
```
