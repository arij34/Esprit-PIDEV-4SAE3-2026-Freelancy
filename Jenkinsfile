pipeline {
    ...
    environment {
        DOCKER_IMAGE  = "ameni221/api-gateway:v${BUILD_NUMBER}"
        SERVICE_DIR   = "api-gateway"
        SONAR_ORG     = "ameny323"
        SONAR_PROJECT = "ameny323_api-gateway"
    }
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                sh """
                    sed -i 's|ameni221/api-gateway:.*|${DOCKER_IMAGE}|' docker-compose.yml
    ...
    post {
        success { echo '✅ api-gateway deployed successfully.' }
